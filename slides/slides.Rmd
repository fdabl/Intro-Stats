...
## Why most published research is false
```{r, echo = FALSE}
table <- rbind(c('Reject Null Hypothesis', '\alpha', '1 - \beta'), c('Do not reject Null', '1 - \alpha', '\beta'))
table
```
...
p(y' = 1|d) &= \int_{\theta} p(y' = 1, \theta|d) \mathrm{d}\theta \\
            ...
            &= \int_{\theta} \theta^{y'} (1 - \theta)^{1 - y'} \frac{1}{\mathcal{B}(y + \alpha, n - y + \beta)} \theta^{\alpha + y - 1}(1 - \theta)^{n - y + \beta - 1}\mathrm{d}\theta
\end{split}
$$
