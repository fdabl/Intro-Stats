...
- **1a.** History of statistics
    - Relation to science
    - Controversies and enigmatic players
...
![](images/uniform-normal-models.png)
...
f(y; \theta, n) &= {n \choose y} \theta^{\sum^n_{i = 1} x_i} (1 - \theta)^{\sum^n_{i = 1} 1 - x_i} \hspace{3em} \text{Neglecting order} \\[1ex]
                &= {n \choose y} \theta^{y} (1 - \theta)^{n - y} \hspace{7em} Y = \sum^n_{i=1} x_i \\
\end{align*}
$$
...
ggplot(data.frame(x = c(0, 1)), aes(x = x)) +
  ...
  geom_segment(x = p1, y = -1, xend = p1, yend = llh.p1, aes(col = 'red'), linetype = 'dashed') +
  geom_segment(x = p2, y = -1, xend = p2, yend = llh.p2, aes(col = 'blue'), linetype = 'dashed') +
...
-----------       --------------------   --------------------
Estimation        Maximum Likelihood     Update beliefs using **Bayes' rule**
Testing           *p*-value              Compare model predictions using **Bayes' rule**
...
- Use the maximum likelihood estimate $\hat{\theta} = \frac{y}{n}$
...
$$
p(y) = \int_{\Theta} f(y; \theta)p(\theta) \mathrm{d}\theta
$$
...
$$
\begin{split}
p(\theta|y) &= \frac{f(y; \theta)p(\theta)}{\int_{\Theta}f(y; \theta)p(\theta)\mathrm{d}\theta} \\[1ex]
p(\theta|y) &= \frac{{n \choose y} \theta^y (1 - \theta)^{n - y} \frac{1}{\mathcal{B}(a, b)} \theta^{\alpha - 1}(1 - \theta)^{\beta - 1}}{\int_{\Theta}{n \choose y} \theta^y (1 - \theta)^{n - y} \frac{1}{\mathcal{B}(a, b)} \theta^{\alpha - 1}(1 - \theta)^{\beta - 1}\mathrm{d}\theta}
\end{split}
$$
...
$$
\begin{split}
p(y|M_0) &= \int_{\Theta} p(y, \theta|M_0)\mathrm{d}\theta \\[1ex]
         &= \int_{\Theta} p(y|M_0, \theta)p(\theta|M_0)\mathrm{d}\theta \\[1ex]
         &= f(y|M_0, \theta = .5) \\[2ex]
...
$$
\begin{split}
p(y|M_1) &= \int_{\Theta} p(y, \theta|M_1)\mathrm{d}\theta \\[1ex]
         &= \int_{\Theta} p(y|M_1, \theta)p(\theta|M_1)\mathrm{d}\theta \\[1ex]
         &= \int_{\Theta} {20 \choose 15} \theta^{15} (1 - \theta)^{20 - 15} \frac{1}{\mathcal{B}(a, b)}\theta^{\alpha - 1}(1 - \theta)^{\beta - 1}\mathrm{d}\theta \\[1ex]
         &\approx \frac{1}{N} \sum_{\theta \sim \text{Beta}(\alpha, \beta)}^N {20 \choose 15} \theta^{15} (1 - \theta)^{20 - 15} \hspace{3em} \text{Monte Carlo integration} \\[1ex]
         &\approx 0.047
\end{split}
$$
...
## Simulator sickness and age
- What did we actually do? What is our statistical model?
![](images/simulator-sickess-jasp.png)

## Simulator sickness and age
- Our statistical model is a multivariate Gaussian distribution for the standardized variable $X = [x_1, x_2]$
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        sliderInput("rho", label = HTML('$$\\rho$$'),
                    min = 0, max = .99, value = 0, step = 0.01)
        ),
      mainPanel(
...
## Simulator sickness and age
- The shape of the multivariate Gaussian is determined by
...
## Simulator sickness and age
- Again, $M_1$ is a bit more complicated as it involves integration

$$
\begin{split}
p(d|M_1) &= \int_{\rho} p(d, \rho|M_1)\mathrm{d}\rho \\[1ex]
         &= \int_{\rho} p(d|M_1, \rho)p(\rho|M_1)\mathrm{d}\rho \\[1ex]
         &= \int_{\rho} f(x;\Sigma)\text{Beta}(\alpha, \beta)\mathrm{d}\rho \\[1ex]
         &= \int_{\rho} \frac{1}{\sqrt{2\pi}} |\Sigma|^{-1} \exp \big (\frac{1}{2}x^T\Sigma^{-1}x \big) \frac{1}{\mathcal{B}(\alpha, \beta)}\rho^{\alpha - 1}(1 - \rho)^{\beta - 1} \mathrm{d}\rho \\[1ex]
         &\approx \frac{1}{N} \sum_{\rho_i \sim \text{Beta}(\alpha, \beta)} ^N f(x;\Sigma_{\rho_i})  \hspace{3em} \text{Monte Carlo integration} \\[1ex]
         &\approx 8.917\mathrm{e}^{-36}
\end{split}
$$
...
$$
\text{BF}_{01} = \frac{8.414\mathrm{e}^{-35}}{8.917\mathrm{e}^{-36}} = 9.436
$$

- But wait! With JASP, we arrived at 

$$
\text{BF}_{01} = 1.978
$$

- How is that possible? We have used the same prior for $M_1$: $\rho \sim \text{Beta}(1, 1)$!


## Simulator sickness and age
- <span style = "color: red">The statistical models differed!</span>
    - Pearson correlation does not require the variables to be normally distributed
    - It only requires continuous random variables
    - JASP did not make the assumption of normality; I did
- Learning outcome: <span style = "color: red">The statistical model is at least as important as the prior!</span>
