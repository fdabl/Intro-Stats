...
- It is a branch of mathematics, and as such uncontroversial
...
## Statistical models
- I used a normal distribution for the data $d$, i.e.
$$
f(x; \mu, \sigma) = \frac{1}{\sqrt{2\pi\sigma^2}} \exp \big (-\frac{1}{2\sigma} (x - \mu)^2 \big)
$$
- Now I can describe the data with just two numbers --- the mean $\mu$ and the variance $\sigma^2$
```{r}
c(mean(x), var(x))
```
...
- Note that our statistical model need be appropriate; in fact, it can be horribly misspecified!
...
- gaussian and uniform graphical model
   ...
   server = function(input, output) {
      plot_dat <- function(n, p) {
        ...
        ggplot(d, aes(x = x, y = y)) +
          ...
          xlab('y') +
          ylab('Density') +
          ggtitle(paste0('Data distribution for theta = ', round(p, 2))) +
   ...
   server = function(input, output) {
      plot.LR <- function(y, n, p1, p2) {
        ...
        ggplot(data.frame(x = c(0, 1)), aes(x = x)) +
          ...
          ylab('Likelihood') +
          xlab(expression(theta)) +
          ggtitle(paste0('Likelihood ratio of f(x; theta_1) / f(x; theta_2) = ', get.LR(y, n, p1, p2)))
      }
...
> We assume that we sample repeatedly from a population; each time we compute an estimate based on
> the sample. The distribution of these estimates --- the sampling distribution --- gives the precision
> of the specific estimate.
...
## Bread and Butter: Estimation
```{r}
sampling_distribution <- apply(replicate(1000, conduct_experiment(y, n)), 2, mean)
```

```{r, echo = FALSE}
hist(sampling_distribution, col = 'grey70', xlab = expression(theta), main = 'Sampling distribution of theta')
```
...
plot_cis <- function(cis, times) {
  ...
  plot(x, z, type = 'n', xlab = 'i-th repeated sample',
  ...
  ui = shinyUI(fluidPage(
    sidebarLayout(
      sidebarPanel(
        ...
        sliderInput("k", label = "y", min = 1, max = 50, value = 7, step = 1),
...
## Bayesian testing: Extending likelihoods
